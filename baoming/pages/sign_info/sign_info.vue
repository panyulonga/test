<template>
	<view class="content">
		...
		<view class="main">
			<view class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">选择赛区</text>

				</view>
				<view class="choice">
					<view class="uni-form-item uni-column" @click="maskClick">
						<picker disabled @change="bindPickerChange" :range="array">
							<label class="">{{array[num]}}</label>
						</picker>
					</view>
					<view class="arrow">
						<image src="../../static/assets/arrow-down.png" mode="widthFix"></image>
					</view>
				</view>
			</view>
			<view class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">作品名称</text>
				</view>
				<input type="text" placeholder="请填写您的作品名称" v-model="works" />
			</view>
			<view class="mian-list" style="display: block;">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">作品封面</text>
				</view>
				<view class="evaluate-image">

					<view v-if="list_img.path==''" class="evaluate-icon" @click="uploadImg('logo','member_cover')">
						<image src="../../static/assets/camera.png" mode="widthFix"></image>
						<text>图片上传 不超过10M</text>
					</view>
					<view v-if="list_img.path!=''" class="val_images">
						<image :src="list_img.path" mode=""></image>
						<view class="del_icon" @click="del_img(0)">
			...
			<view class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">作品简介</text>
				</view>
				<input type="text" disabled placeholder="   " />
				<textarea maxlength="500" placeholder="填写简介（500字以内）" v-model="briefs" />
			</view>


			<view v-if="num==0" class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">公众号名称：</text>
				</view>
				<input type="text" placeholder="请填写您的公众号名称" v-model="account_name" />
			</view>
			<view v-if="num==0" class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">公众号微信号：</text>
				</view>
				<input type="text" placeholder="请填写您的公众号微信号" v-model="account_id" />
			</view>

			<view v-if="num==0" class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">文章链接</text>
				</view>
				<input type="text" placeholder="请填写您的文章链接" v-model="link" />
			</view>

			<view v-if="num==1" class="mian-list" style="display: block;">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">上传视频</text>
				</view>
				<view class="evaluate-image">

					<view v-if="src.path==''" class="evaluate-icon" @click="addVideoFun">
						<image src="../../static/assets/film.png" mode="widthFix"></image>
						<text style="font-size: 12px;">视频上传 不超过500M</text>
					</view>
					<view v-if="src.path!=''" class="val_images">
						<video :src="src.path" style="video"></video>
						<view class="del_icon" @click="delImg()">
							<image src="../../static/assets/del_icon.png" mode="widthFix"></image>
						</view>
					</view>
				</view>
			</view>
			<view v-if="num==2" class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">APPID</text>
				</view>
				<input type="text" placeholder="请填写您小程序的APPID" v-model="fillinAppId" />
			</view>
			<view v-if="num==2" class="mian-list">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">小程序页面路径</text>
				</view>
				<input type="text" disabled placeholder="  " />
				<textarea placeholder="填写小程序路径..." v-model="xcx_src" />
			</view>
			<view v-if="num!=1" class="mian-list" style="display: block;">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">{{num==2?'小程序':'公众号'}}二维码</text>
				</view>
				<view class="evaluate-image">

					<view v-if="list_img2.path==''" class="evaluate-icon" @click="uploadImg('logo','member_qrcode')">
						<image src="../../static/assets/camera.png" mode="widthFix"></image>
						<text style="font-size: 12px;">图片上传 不超过10M</text>
					</view>
					<view v-if="list_img2.path!=''" class="val_images">
						<image :src="list_img2.path" mode=""></image>
						<view class="del_icon" @click="del_img(1)">
							<image src="../../static/assets/del_icon.png" mode="widthFix"></image>
						</view>
					</view>
				</view>
			...
			<view class="mian-list" style="display: block;">
				<view class="list-left">
					<text class="mandatory im-red">*</text>
					<text class="tips">承诺书</text>
				</view>
				<view class="evaluate-image">

					<view v-if="list_img3.path==''" class="evaluate-icon" @click="uploadImg('guarantee','member_statement')">
						<image src="../../static/assets/camera.png" mode="widthFix"></image>
						<text>图片上传 不超过10M</text>
					</view>
					<view v-if="list_img3.path!=''" class="val_images">
						<image :src="list_img3.path" mode=""></image>
						<view class="del_icon" @click="del_img(2)">
							<image src="../../static/assets/del_icon.png" mode="widthFix"></image>
						</view>
					</view>
				</view>
		...
		<view class="btm-info">
			<view class="btm-btn" @click="unloadInfo()">{{btmText}}</view>
		</view>

		<view v-if="mask" class="select-content">
			<view>请选择您的作品提交赛区</view>
			<button style="background-color: #159AFF;" @click="articleDetails('article')">图文赛区</button>
			<button style="background-color: #FCA552;" @click="articleDetails('video')">视频赛区</button>
			<button style="background-color: #AA74FE;" @click="articleDetails('miniapp')">小程序赛区</button>
		</view>
		<view v-if="mask" class="mask-show"></view>
	</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.type) {
				this.mask = false;
				this.articleDetails(options.type)
			}
		...
		methods: {
			...
			articleDetails(type) {
				...
				this.list_img2.path = '';
				this.list_img3.path = '';
				this.personInfo.type = type;
				this.mask = false;
				console.log(15151);
				uni.request({
					url: common.WEBSITE_URL + '/api/competition/getAllSubmission?token=' + this.personInfo.token +
						'&channel=' + common.CHANNEL + '&type=' + this.personInfo.type,
					success: (res) => {
						console.log(res.data)
							console.log(this.personInfo.type,'-----',uni.getStorageSync('type', 'data'))
						if(this.personInfo.type == uni.getStorageSync('type', 'data')) {
							this.btmText = '修改作品';
						}
						else {
							this.btmText = '提交作品';
						}
						if (res.data.data[this.personInfo.type][0]) {

							this.personInfo.method = 'update';
							

							this.numType = res.data.data[this.personInfo.type][0].type;
							this.typeBl = true;


							this.works = res.data.data[this.personInfo.type][0].name;
							this.briefs = res.data.data[this.personInfo.type][0].description;
							this.link = res.data.data[this.personInfo.type][0].url;
							this.account_name = res.data.data[this.personInfo.type][0].account_name;
							this.account_id = res.data.data[this.personInfo.type][0].account_id;
							this.fillinAppId = res.data.data[this.personInfo.type][0].appid;
							this.xcx_src = res.data.data[this.personInfo.type][0].path;
							this.list_img.path = [(res.data.data[this.personInfo.type][0].cover)];
							this.list_img2.path = [(res.data.data[this.personInfo.type][0].qrcode)];
							this.list_img3.path = [(res.data.data[this.personInfo.type][0].statement)];
							this.src.path = res.data.data[this.personInfo.type][0].video;

						} else {
							this.personInfo.method = 'create';
						}
						if (this.personInfo.type == 'article') {
							this.num = 0;
						} else if (this.personInfo.type == 'video') {
							this.num = 1;
						} else if (this.personInfo.type == 'miniapp') {
							this.num = 2;
						}
						if (parseInt(res.data.data[this.personInfo.type][0].audit_status) == 2) {
							uni.showModal({
								title: "已审核",
								content: "您的作品已通过审核！",
								showCancel: false,
							});
						} else if (parseInt(res.data.data[this.personInfo.type][0].audit_status) == -1) {
							console.log(res.data.data[this.personInfo.type][0].audit_status);
							uni.showModal({
								title: "未通过",
								content: '原因：' +  res.data.data[this.personInfo.type][0].reject_reason + '。请修改后重新提交',
								showCancel: false,
							});

						} else if (parseInt(res.data.data[this.personInfo.type][0].audit_status) == 0) {
							uni.showModal({
								title: "待审核",
								content: "您的作品已提交",
								showCancel: false,
			...
			unloadInfo() {
				console.log(this.list_img3)
				if (this.list_img3.path == '') {
					console.log('jinlail')
					uni.showToast({
						icon: 'none',
						title: '请把信息填写完整'
				...
				if (this.personInfo.method == 'update' && this.list_img3.id === "") {
					
				} else {
					this.info_data.statement = this.list_img3.id;
				}
				if (this.personInfo.method == 'update' && this.list_img.id === "") {
					console.log('我进来啦')

				} else {
					this.info_data.cover = this.list_img.id;
				}
				this.personInfo.type = this.num == 0 ? 'article' : (this.num == 1 ? 'video' : 'miniapp');
				this.info_data.name = this.works;
				this.info_data.description = this.briefs;
				
				
				
				
				if (this.num == 0) {
					if (this.works == '' || this.briefs == '' || this.link == '' || this.list_img.path == '' || this
						.list_img2.path == '' || this.account_name == '' || this.account_id == '') {
						uni.showToast({
							icon: 'none',
							title: '请把信息填写完整'
						})
					} else {
						if (this.personInfo.method == 'update' && this.list_img2.id === "") {

						} else {
							this.info_data.qrcode = this.list_img2.id;
						}
						this.info_data.url = this.link;
						this.info_data.account_name = this.account_name;
						this.info_data.account_id = this.account_id;
						this.submitInfo();

					}
				} else if (this.num == 1) {
					if (this.works == '' || this.briefs == '' || this.src.path == '' || this.list_img.path == '') {
						uni.showToast({
							icon: 'none',
							title: '请把信息填写完整'
						})
						console.log(this.src.path, this.list_img2.path)
					} else {
						if (this.personInfo.method == 'update' && this.src.id === "") {

						} else {
							this.info_data.video = this.src.id;
						}
						this.submitInfo();
					}
				} else if (this.num == 2) {
					if (this.works == '' || this.briefs == '' || this.fillinAppId == '' || this.list_img.path == '' || this
						.xcx_src == '' || this.list_img2.path == '') {
						uni.showToast({
							icon: 'none',
							title: '请把信息填写完整'
						})
						...
						this.info_data.appid = this.fillinAppId;
						this.info_data.path = this.xcx_src;
						this.submitInfo();
			...
			submitInfo() {
				...
				var shuju = this.info_data;
				uni.request({
					method: 'POST',
					url: common.WEBSITE_URL + '/api/competition/submission?token=' + this.personInfo.token +
						'&channel=' + common.CHANNEL + '&method=' + this.personInfo.method + '&type=' + this
						.personInfo.type,
					data: shuju,
					success: (res) => {
						console.log(res.data, '-----', this.info_data)

						if (res.data.code == 0) {
							
							uni.setStorageSync('type',this.personInfo.type);
							uni.showModal({
								title: this.typeBl ? "修改成功" : '提交成功',
								showCancel: false,
								success() {
									uni.navigateTo({
			...
			bindPickerChange(e) { //改变的事件名
				...
				this.num = e.target.value //将数组改变索引赋给定义的index变量
				this.jg = this.array[this.num] //将array【改变索引】的值赋给定义的jg变量
					...
					success: function(res) {
						...
						uni.uploadFile({
							url: common.WEBSITE_URL + '/api/api/multimediaUpload?channel=' + common
								.CHANNEL + '&token=' + self.personInfo.token +
								'&key=video&module=member_video&type=video', //接口地址
							...
							success: (uploadFileRes) => {
								...
								uni.hideLoading();
								self.src.path = bold.data.url;
								self.src.id = bold.data.file_id;
						...
						res.tempFilePaths.forEach(path => {
							uni.uploadFile({
								url: common.WEBSITE_URL + '/api/api/multimediaUpload?channel=' +
									...
									'&key=' + key + '&module=' + moduleName + '&type=image',
								filePath: path,
								name: key,
								success(res) {
									if (res.data) {
										try {
											...
											if (ret.code != 0) {
												...
												console.log('进来了')
												uni.hideLoading();
												if (moduleName == 'member_cover') {
													that.list_img.path = ret.data.url;
													that.list_img.id = ret.data.file_id;
												} 
												else if(moduleName == 'member_qrcode'){
													that.list_img2.path = ret.data.url;
													that.list_img2.id = ret.data.file_id;
												}
												else if(moduleName == 'member_statement'){
													that.list_img3.path = ret.data.url;
													that.list_img3.id = ret.data.file_id.toString();
												}
											}
